<template>
  <div class="Hasservice">
    ...
    <div >
      ...
      <div  v-show="showList" class="Hasservice_list" v-for="(item,index) in  finish_cards" :key="index" @click="Hasservice(item.card_id)">
        <div class="Hasservice_listher">
          ...
          <van-rate v-show="rate" v-else ="item.rate==null" :size="14" :count="5" color="#ffa800" readonly void-color="#ffa800"/>
        </div>
    ...
    <div style="text-align: center; padding-top: 12px; height:100%;background: #fff;" v-show="show">
      ...
      <p style=" font-size: 12px;">您还没有已服务工单</p>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      request(days, that) {
        if (days == 100) {
          axios.post(BASE_URL + "/api/get_worker_info", {

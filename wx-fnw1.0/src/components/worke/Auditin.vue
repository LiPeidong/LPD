    ...
    <!-- 审核中 -->
        <div class="Audit">
            <div class="Hasservice_listcen">
                <h2>{{name}}  师傅</h2>
                <p>手机号码：{{phone}}</p>
                <p style="display:flex;flex-wrap:wrap;">服务内容：<span v-for="(item,index) in content" :key="index" style="margin-right:5px;">{{item}}</span></p>
                <p>所属商家：{{merchant}}</p>
                <p style="display:flex;flex-wrap:wrap;">服务范围：<span v-for="(item,index) in service" :key="index" style="margin-right:5px;">{{item}}</span></p>

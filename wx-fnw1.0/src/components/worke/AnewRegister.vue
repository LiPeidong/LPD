...
export default {
  ...
  created() {
    ...
    if (regdata && reguser) {
      ...
      this.form.IDcard = reguser.id_card;
      ...
      this.adds1 = this.adds1.reverse();
      this.adds1 = this.adds1.join(" ");
    } else if (regfailed) {
      this.nickName = regdata.nickname;
      this.form.phone = regfailed.user.phone;
      this.form.IDcard = regfailed.user.id_card;
      this.form.user = regfailed.user.name; //真实姓名
      this.form.shop.name = regfailed.user.service_provider; //服务商
      this.addselect1 = regfailed.list; //四川省-成都市-区
      this.can_areas = regfailed.list.county_list; //区
      for (var i = 0; i < regfailed.user.districts.length; i++) {
        this.adds1.push(regfailed.user.districts[i].name);
      }
      this.adds1 = this.adds1.reverse();
      this.adds1 = this.adds1.join(" ");
    }

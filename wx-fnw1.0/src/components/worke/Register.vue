    ...
    <div id="app" v-cloak>
      <div class="BOXS">
        ...
        <div class="p-10" style="margin: 20px 0;text-align:center;">
          <van-button class="btn-primary" size="large" @click="register()">下一步</van-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    ...
    console.log(regdata.can_areas.county_list)
  ...
  methods: {
    ...
    //   if(!this.form.user){
    //
    //     console.log(document.getElementsByClassName('van-field__control')[1])
    //    console.log( document.getElementsByClassName('van-field__control')[1].getAttribute('class'))
    //    var a = document.getElementsByClassName('van-field__control')[1].getAttribute('class');
    //     // document.getElementsByClassName('van-field__control')[1].setAttribute('class',a+)
    //   }
    //
    // },
    ...
    onConfirm(value) {
      ...
      for (var i = 0; i < value.length; i++) {
        // this.form.listcode.push(value[i].code)
        this.form.listcode.push(value[i].name);
        console.log("value", this.form.listcode);
    ...
    register() {
      ...
      //检验并拼接服务区域
      // console.log(this.can_areas,this.form.adds1)
...
body .van-checkbox__icon--round .van-icon{margin-right: 5px !important;}
